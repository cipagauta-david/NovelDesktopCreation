...
## 6b. Flujo de Datos (Data Flow)

### Flujo de Persistencia (Estado)

```
┌─────────────┐     Intent          ┌───────────────┐     IPC/Adapter     ┌───────────────┐
│ UI/Renderer │ ──────────────────► │   Worker/TS   │ ──────────────────► │ StateStorage  │
│ (React App) │                     │ (Sync Engine) │                     │ Adapter       │
└─────────────┘        Event        └───────────────┘                     └───────┬───────┘
      ▲                  │                                                        │
      │                  ▼                                                        ▼
      │           ┌──────────────┐                                        ┌────────────────┐
      └────────── │  ChangeEvent │                                        │ Desktop: SQLite│
                  │  (Append)    │                                        │ Web: IndexedDB │
                  └──────────────┘                                        └────────────────┘
```
...
```
┌──────────────┐     PersistedState     ┌───────────────┐   deriveSyncOperations   ┌─────────────┐
│   Workspace  │ ─────────────────────► │  Sync Engine  │ ───────────────────────► │  Sync Queue │
│   (State)    │                        │               │                          │  (SQLite)   │
└──────────────┘                        └───────────────┘                          └──────┬──────┘
                                                                                          │
                                                                                          ▼
┌─────────────┐   mergePersistedStates  ┌───────────────┐                          ┌──────────────┐
│ Remote API  │ ◄────────────────────── │  Flush Queue  │ ◄─────────────────────── │  Retry/      │
│ (/sync/v2)  │                         │               │                          │  Exponential │
└──────┬──────┘                         └───────────────┘                          └──────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│                         Merge Strategy (CRDT + LWW)                          │
│  - entity.content: CRDT text merge                                           │
│  - entity.metadata: LWW by updatedAt                                         │
│  - projects/relations: LWW by updatedAt                                      │
└──────────────────────────────────────────────────────────────────────────────┘
```

### Flujo FTS (Full-Text Search)

```
┌──────────────┐   Entidades    ┌──────────────┐   Diff     ┌─────────────┐
│  Persisted   │ ────────────►  │  Flatten     │ ─────────► │  FTS Index  │
│   State      │                │  Entities    │            │  (FTS5)     │
└──────────────┘                └──────────────┘            └─────────────┘
                                       │                           │
                                       ▼                           ▼
